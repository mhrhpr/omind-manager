...
from .models import AnalysisRecord, DecisionRecord, SessionRecord, UserRecord, WorkspaceRecord
...
def auth_workspace_for_analysis(analysis_id: UUID, authorization: str | None, db: Session) -> WorkspaceRecord:
    ...
    if workspace.auth_token_hash and secrets.compare_digest(workspace.auth_token_hash, hash_token(token)): return workspace
    session = current_session(token, db)
    if session is None or session.workspace_id != workspace.id: raise HTTPException(403, 'workspace access denied')
    return workspace
...
@app.get('/health')
def health() -> dict[str, str]: return {'status': 'ok', 'service': 'omind-api', 'version': '1.2.0'}

@app.post('/auth/signup', response_model=AuthResponse)
def signup(payload: SignupPayload, db: Session = Depends(get_db)) -> AuthResponse:
    email = payload.email.strip().lower()
    if db.scalar(select(UserRecord).where(UserRecord.email == email)) is not None: raise HTTPException(409, 'ایمیل قبلاً ثبت شده است.')
    user = UserRecord(name=payload.name.strip(), email=email, password_hash=hash_password(payload.password)); db.add(user); db.flush()
    workspace = WorkspaceRecord(owner_user_id=user.id, auth_token_hash=None); db.add(workspace); db.flush(); token = create_session(db, user.id, workspace.id)
    db.commit(); db.refresh(user); db.refresh(workspace); return auth_response(user, workspace, token)

@app.post('/auth/login', response_model=AuthResponse)
def login(payload: AuthPayload, db: Session = Depends(get_db)) -> AuthResponse:
    email = payload.email.strip().lower(); user = db.scalar(select(UserRecord).where(UserRecord.email == email))
    if user is None or not verify_password(payload.password, user.password_hash): raise HTTPException(401, 'ایمیل یا رمز عبور نادرست است.')
    ...
    if workspace is None: workspace = WorkspaceRecord(owner_user_id=user.id); db.add(workspace); db.flush()
    token = create_session(db, user.id, workspace.id); db.commit(); db.refresh(workspace); return auth_response(user, workspace, token)

@app.get('/auth/me')
def me(authorization: str | None = Header(default=None), db: Session = Depends(get_db)) -> dict:
    user = current_user(authorization, db); session = current_session(authorization.removeprefix('Bearer ').strip() if authorization else '', db); workspace = db.get(WorkspaceRecord, session.workspace_id) if session else None
    if workspace is None: raise HTTPException(401, 'workspace not found')
...
async def create_analysis(file: UploadFile = File(...), question: str = Form(''), role: str = Form('manager'), workspace_id: UUID = Form(...), authorization: str | None = Header(default=None), db: Session = Depends(get_db)) -> AnalysisResponse:
    ...
    try:
        rows = parse_upload(file.filename, content); result = analyze_rows(rows, question, role); result['charts'] = build_chart_specs(rows, result.get('columns'))
