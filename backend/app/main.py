...
def auth_workspace_for_analysis(analysis_id: UUID, authorization: str | None, db: Session) -> WorkspaceRecord:
    ...
    if workspace is None or not workspace.auth_token_hash:
        raise HTTPException(403, 'workspace access denied')
    token = authorization.removeprefix('Bearer ').strip()
    if not __import__('secrets').compare_digest(workspace.auth_token_hash, hash_token(token)):
        raise HTTPException(403, 'workspace access denied')
    return workspace
...
def get_workspace(workspace_id: UUID, _: WorkspaceRecord = Depends(require_workspace)) -> WorkspaceResponse:
    return workspace_view(_)


@app.get('/workspaces/{workspace_id}/analyses', response_model=list[AnalysisResponse])
def list_analyses(workspace_id: UUID, _: WorkspaceRecord = Depends(require_workspace), db: Session = Depends(get_db)) -> list[AnalysisResponse]:
...
async def create_analysis(file: UploadFile = File(...), question: str = Form(''), workspace_id: UUID = Form(...), authorization: str | None = Header(default=None), db: Session = Depends(get_db)) -> AnalysisResponse:
    ...
    try:
        ...
        result = analyze_rows(rows, question)
...
def list_decisions(workspace_id: UUID, authorization: str | None = Header(default=None), db: Session = Depends(get_db)) -> list[DecisionResponse]:
    ...
    return [DecisionResponse.model_validate(record) for record in db.scalars(stmt).all()]
