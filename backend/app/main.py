...
def auth_workspace_for_analysis(analysis_id: UUID, authorization: str | None, db: Session) -> WorkspaceRecord:
    ...
    if workspace is None or not workspace.auth_token_hash or not secrets.compare_digest(workspace.auth_token_hash, hash_token(token)):
...
def create_workspace(db: Session = Depends(get_db)) -> WorkspaceBootstrap:
    token=issue_token(); record=WorkspaceRecord(auth_token_hash=hash_token(token)); db.add(record); db.commit(); db.refresh(record)
...
async def create_analysis(file: UploadFile=File(...), question: str=Form(''), role: str=Form('manager'), workspace_id: UUID=Form(...), authorization: str|None=Header(default=None), db: Session=Depends(get_db)) -> AnalysisResponse:
    workspace=require_workspace(workspace_id, authorization, db)
    ...
    if not file.filename.lower().endswith(allowed): raise HTTPException(415,'unsupported file type')
    content=await file.read()
    if len(content)>settings.max_upload_bytes: raise HTTPException(413,'file exceeds upload limit')
    try:
        rows=parse_upload(file.filename, content); result=analyze_rows(rows, question, role)
...
def create_decision(payload: DecisionCreate, authorization: str|None=Header(default=None), db: Session=Depends(get_db)) -> DecisionResponse:
    auth_workspace_for_analysis(payload.analysis_id, authorization, db); record=DecisionRecord(**payload.model_dump()); db.add(record); db.commit(); db.refresh(record)
...
def update_outcome(decision_id: UUID, payload: DecisionOutcomeUpdate, authorization: str|None=Header(default=None), db: Session=Depends(get_db)) -> DecisionResponse:
    ...
    auth_workspace_for_analysis(record.analysis_id, authorization, db); record.actual_outcome=payload.actual_outcome; record.lesson=payload.lesson; record.status=payload.status; db.commit(); db.refresh(record)
    return DecisionResponse.model_validate(record)
