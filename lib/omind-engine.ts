...
export type ColumnProfile = {
  name: string;
  role: 'key' | 'date' | 'measure' | 'dimension' | 'text' | 'category';
...
export type Signal = {
  type: 'missingness' | 'outlier' | 'concentration' | 'trend';
  title: string;
  detail: string;
...
export function normalizeRows(rows: Row[]): Row[] {
  return rows.map(row => Object.fromEntries(Object.entries(row).map(([k, v]) => {
    ...
    const western = s.replace(/[۰-۹]/g, d => String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d))).replace(/[٠-٩]/g, d => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d)));
    if (western !== '' && /^-?\d+(\.\d+)?$/.test(western)) return [k, Number(western)];
    return [k, s === '' ? null : s];
  })));
}
...
export function profileRows(input: Row[]): Analysis {
  ...
  const columns: ColumnProfile[] = names.map(name => {
    ...
    const numericRatio = values.length ? values.filter(numeric).length / values.length : 0;
    const dateRatio = values.length ? values.filter(asDate).length / values.length : 0;
    ...
    return { name, role, type, missing, unique };
  });

  const duplicateCount = rows.length - new Set(rows.map(r => JSON.stringify(r))).size;
  const missingCells = columns.reduce((sum, c) => sum + c.missing, 0);
  ...
  const health = Math.max(0, Math.round(100 - missingRate * 45 - duplicateRate * 30));
  ...
  if (duplicateCount > 0) signals.push({ type: 'outlier', title: 'ردیف تکراری', detail: `${duplicateCount} ردیف تکراری شناسایی شد.`, priority: duplicateRate > .05 ? 'high' : 'medium', score: Math.round(duplicateRate * 100) });

  for (const c of columns.filter(c => c.type === 'number').slice(0, 6)) {
    const vals = rows.map(r => asNumber(r[c.name])).filter(Number.isFinite) as number[];
    if (vals.length < 5) continue;
    const mean = vals.reduce((a, b) => a + b, 0) / vals.length;
    const variance = vals.reduce((a, b) => a + (b - mean) ** 2, 0) / vals.length;
    const sd = Math.sqrt(variance);
    const outliers = sd ? vals.filter(v => Math.abs(v - mean) > 3 * sd).length : 0;
    if (outliers) signals.push({ type: 'outlier', title: `ناهنجاری در ${c.name}`, detail: `${outliers} مقدار بیش از ۳ انحراف معیار از میانگین فاصله دارد.`, priority: outliers / vals.length > .05 ? 'high' : 'medium', score: Math.min(100, Math.round(outliers / vals.length * 1000)) });
  }

  const measures = columns.filter(c => c.role === 'measure').map(c => c.name);
  const categories = columns.filter(c => c.role === 'category').map(c => c.name);
  if (measures.length && categories.length) {
