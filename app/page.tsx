...
const tabs = ['workspace', 'history', 'pricing'] as const;
...
const PERSONAS: Record<string, FilePersona> = {
  excel: { key: 'excel', label: 'Excel', icon: '▦', accent: '#21A366', message: 'اکسل را گرفتم. بگذار ببینم چه چیزی در جدول پنهان شده.' },
...
async function parseFile(file: File): Promise<Row[]> {
  ...
  if (name.endsWith('.json')) { const parsed = JSON.parse(new TextDecoder().decode(buffer)); return Array.isArray(parsed) ? parsed : (parsed.data ?? []); }
  ...
  async function analyze() {
    ...
    try {
      ...
      if (API_BASE && workspaceId) {
        ...
        if (!saved.ok) throw new Error(saved.status === 402 ? 'سهمیه نسخه آزمایشی تمام شده است.' : 'ذخیره تحلیل در سرور ناموفق بود.');
        const server = await saved.json();
        const quotaResponse = await fetch(`${API_BASE}/workspaces/${workspaceId}`);
        if (quotaResponse.ok) setServerRemaining((await quotaResponse.json()).remaining);
...
function AnalysisView({ analysis, question }: { analysis: Analysis; question: string }) { return <section className="analysis-section"><div className="result-header"><div><span className="eyebrow">DECISION ROOM</span><h2>{question}</h2></div><div className="score"><b>{analysis.health}</b><span>DATA HEALTH</span></div></div><div className="metric-grid"><Metric label="ردیف" value={analysis.rows.toLocaleString('fa-IR')} /><Metric label="ستون" value={analysis.columns.length.toLocaleString('fa-IR')} /><Metric label="سیگنال" value={analysis.signals.length.toLocaleString('fa-IR')} /><Metric label="Confidence" value={`${analysis.confidence}%`} /></div><div className="result-grid"><ResultCard title="Key Signals">{analysis.signals.length ? analysis.signals.map(s => <div className="signal" key={s.title}><span className={`priority ${s.priority}`}>{s.priority === 'high' ? 'HIGH' : s.priority === 'medium' ? 'MED' : 'LOW'}</span><div><b>{s.title}</b><p>{s.detail}</p></div></div>) : <p>سیگنال قابل توجهی با قواعد فعلی شناسایی نشد.</p>}</ResultCard><ResultCard title="Top Priorities">{analysis.priorities.map(x => <p className="bullet" key={x}>{x}</p>)}</ResultCard><ResultCard title="Hypotheses">{analysis.hypotheses.map(x => <p className="hypothesis" key={x}>{x}</p>)}</ResultCard><ResultCard title="Recommended Actions">{analysis.actions.map((x, i) => <p className="action" key={x}><b>{i + 1}</b>{x}</p>)}</ResultCard></div><details className="trace"><summary>Reasoning Trace — 12 modules</summary><div>{analysis.trace.map(x => <span key={x}>{x}</span>)}</div></details><details className="trace reasoning-detail"><summary>Execution Detail — confidence / outputs</summary><div>{analysis.reasoning.steps.map(step => <article key={step.module}><b>{step.module}</b><small>confidence {step.confidence}%</small>{step.output.map((out, i) => <p key={`${step.module}-${i}`}>{out}</p>)}</article>)}</div></details><div className="columns"><h3>Data Model</h3>{analysis.columns.map(c => <div key={c.name}><b>{c.name}</b><span>{c.role}</span><small>{c.type} · missing {c.missing}</small></div>)}</div></section>; }
